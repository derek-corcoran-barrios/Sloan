...
ShotSeasonGraph <- function(SeasondataOff, nbins = 30, maxsize = 7, quant = 0.7, type = "PPS", MAX_Y = 270) {
  ...
  ##Total NBA data
  Totalhex <- makeHexData(SeasondataOff)
  Totalhex$ST <- ifelse(Totalhex$ST <= quantile(Totalhex$ST)[2], 0.12,
                    ifelse(Totalhex$ST > quantile(Totalhex$ST)[2] & Totalhex$ST <= quantile(Totalhex$ST)[3] , 0.25 ,
                           ifelse(Totalhex$ST > quantile(Totalhex$ST)[3] & Totalhex$ST <= quantile(Totalhex$ST)[4] , 0.5 ,        
                                  1)))
  ...
  return(list(GRAPH = GRAPH, DF = Totalhex))
}


DF <-  ShotSeasonGraph(shotDataTotal2017, quant = 0.4, MAX_Y = 270)

Test <- DF$DF

Test$ST <- ifelse(Test$ST <= quantile(Test$ST)[2], 0.25,
           ifelse(Test$ST > quantile(Test$ST)[2] & Test$ST <= quantile(Test$ST)[3] , 0.5 ,
           ifelse(Test$ST > quantile(Test$ST)[3] & Test$ST <= quantile(Test$ST)[4] , 0.75 ,        
                  1)))
