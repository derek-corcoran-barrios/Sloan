...
These are the new projected standings predicted by our spatial-based algorithm as of `r format(Sys.time(), '%d %B, %Y')`

```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE}
if (!require("pacman")) install.packages("pacman")

pacman::p_load(rjson, grid, gridExtra, png, RCurl, ggplot2, jpeg, hexbin, sp, knitr, raster, rasterVis, dplyr, DT)
...
ForecastWL <- predict(BRT2, data.frame(defAPPS = unlist(future_games[,4]), offAPPS = unlist(future_games[,5])), type = "prob")
...
for (i in 1:1000){
  ...
  for(j in 1:nrow(future_games)){
    Results[[j]] <- sample(c("W", "L"), size = 1, replace = TRUE, prob = future_games[j,c(7,8)])
  }
  Seasons[[i]] <- future_games
  Seasons[[i]]$Home <- unlist(Results)
  Seasons[[i]]$Visit <- ifelse(Seasons[[i]]$Home == "W", "W", "L")
}
...
for (i in 1:length(Seasons)){
  ...
  Home[[i]] <- cbind(Seasons[[i]]$`Home/Neutral`, Seasons[[i]]$Home)

  colnames(Home[[i]]) <- c("Team", "Result")

  Visit[[i]] <- cbind(Seasons[[i]]$`Visitor/Neutral`, Seasons[[i]]$Visit)

  colnames(Visit[[i]]) <- c("Team", "Result")

  AddedStand[[i]] <- data.frame(rbind(Home[[i]], Visit[[i]]))
#Wins
  AddedStand_W[[i]] <- dplyr::filter(AddedStand[[i]], Result == "W")

  AddedStand_W[[i]] <- group_by(AddedStand_W[[i]], Team)

  AddedStand_W[[i]] <- dplyr::summarize(AddedStand_W[[i]], W = n())

#Loses

  AddedStand_L[[i]] <- dplyr::filter(AddedStand[[i]], Result == "L")

  AddedStand_L[[i]] <- group_by(AddedStand_L[[i]], Team)

  AddedStand_L[[i]] <- dplyr::summarize(AddedStand_L[[i]], L = n())

  AddedStand[[i]] <- merge.data.frame(AddedStand_W[[i]], AddedStand_L[[i]], all = TRUE)

}

AddedStand <- do.call("rbind", AddedStand)

hist(dplyr::filter(AddedStand, Team == "GSW")$W)

AddedStand<- dplyr::group_by(AddedStand, Team)

AddedStand <- dplyr::summarise(AddedStand, W = mean(W), L = mean(L))
